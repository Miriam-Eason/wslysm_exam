...
type Student = { id: number; studentNo: string; name: string };
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success(
        ...
        d.softDeleted
          ? { description: `其中 ${d.softDeleted} 名有作答记录，已停用（软删除）` }
          : undefined,
      );
      setSelected(new Set());
      ...
      <AlertDialog open={confirmOpen} onOpenChange={(o) => !o && setConfirmOpen(false)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              将删除 {selected.size} 名学生的身份与班级关系。无作答记录的学生
              <span className="font-medium text-on-surface">直接删除</span>；
              已有作答记录的将<span className="font-medium text-on-surface">停用（软删除）</span>以保护成绩数据。此操作不可撤销。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
