    ...
    <>
      ...
      {classes.length === 0 ? (
        <Card className="flex flex-col items-center gap-3 px-6 py-16 text-center">
          ...
          <h3 className="text-lg font-semibold text-on-surface">还没有班级</h3>
          <p className="max-w-xs text-sm text-on-surface-variant">
            创建第一个班级，然后用 Excel 模板批量导入学生名单。
          </p>
        ...
        <DialogContent>
          <form onSubmit={handleCreate} className="flex flex-col gap-5">
            <DialogHeader>
              ...
              <DialogDescription>为班级取一个名称，例如「高一(1)班」。</DialogDescription>
            </DialogHeader>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              将删除「{deleteTarget?.name}」及其学生成员关系（共 {deleteTarget?.studentCount} 人），
