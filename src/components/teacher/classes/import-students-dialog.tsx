...
import { Upload, FileUp, CheckCircle2, AlertTriangle, XCircle, Info } from "lucide-react";
...
export function ImportStudentsDialog({ classId }: { classId: number }) {
  ...
  async function runPreview(f: File) {
    ...
    try {
      ...
      fd.append("file", f);
      const res = await fetch(`/api/classes/${classId}/students/preview`, {
        method: "POST",
        body: fd,
      });
  ...
  async function confirmImport() {
    ...
    try {
      ...
      const d = json.data;
      toast.success(`导入完成：新增 ${d.createdStudents} 人 · 加入本班 ${d.enrolled} 人`, {
        description:
    ...
    >
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            上传由「下载导入模板」得到的 .xlsx 文件（仅学号、姓名两列）。系统会先预检，确认无误后再导入。
          </DialogDescription>
        </DialogHeader>
        ...
        >
          ...
          <span className="text-sm font-medium text-on-surface">
            {dragging ? "松开以导入" : file ? file.name : "点击选择，或拖拽 .xlsx 文件到此处"}
          </span>
        ...
        {preview && (
          <div className="flex max-h-[42vh] flex-col gap-3 overflow-y-auto">
            ...
            <Section
              title="已在本班（无需重复导入）"
              tone="muted"
              rows={preview.alreadyInClass.map((r) => ({ rowNo: r.rowNo, text: `${r.studentNo} ${r.name}` }))}
            />
          </div>
        )}
        ...
        <DialogFooter>
          ...
          <Button onClick={confirmImport} disabled={!canImport}>
            {phase === "importing"
              ? "导入中…"
              : preview
                ? `确认导入 ${preview.summary.importable} 人`
                : "确认导入"}
          </Button>
        </DialogFooter>
      </DialogContent>
